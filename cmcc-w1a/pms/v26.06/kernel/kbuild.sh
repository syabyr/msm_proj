#!/bin/sh
# =============================================================================
# In-container build entrypoint for linux-6.12.1-msm8916 (armv7).
#
...
#   /cfg   dir holding config-postmarketos-qcom-msm8916.armv7, read-only
#
# Environment:
...
# armv7 only (32-bit ARM). ARCH=arm; the bare-metal EABI toolchain matches
# CONFIG_AEABI=y in the pmOS config.
KARCH=arm
...
#
# armv7 has no CONFIG_EFI_ZBOOT, so (per the APKBUILD `else` branch) zinstall is
# added to the targets: arch/arm/boot/install.sh copies arch/arm/boot/zImage to
# boot/vmlinuz-<kernel.release> (and System.map-<kernel.release>).
package() {
    log "collecting artifacts -> $OUT"
    mkdir -p "$OUT/boot" "$OUT/usr/share/kernel/$FLAVOR"

    # Install targets, branching like the APKBUILD package():
    ...
    if [ -e "$ZBOOT_IMG" ]; then
        ...
        # armv7 (no ZBOOT): zinstall produces boot/vmlinuz-<kernel.release>
        _install_targets="zinstall modules_install dtbs_install"
    fi

    # unquoted: _install_targets must split into separate make targets
